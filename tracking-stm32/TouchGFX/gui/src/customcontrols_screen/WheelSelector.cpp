#include <gui/customcontrols_screen/WheelSelector.hpp>
...
void WheelSelector::setup(int width, int height, int textXPosition, int textYPosition, TEXTS wheelText)
{
    ...
    spaceBetweenTextRows = text.getTypedText().getFont()->getHeight() * 2; // For blank lines between texts;
...
void WheelSelector::textMoveEndedHandler(const MoveAnimator<TextArea>& textArea)
{
    ...
    {
        if (!textArea.isMoveAnimationRunning() && !textSelected.isMoveAnimationRunning())
        {
            snap();
        }
    }
